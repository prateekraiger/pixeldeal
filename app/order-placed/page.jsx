...
const OrderPlaced = () => {
  ...
  if (!orderDetails || !Array.isArray(orderItems)) {
    return <div className="flex justify-center items-center h-screen">Loading order details or order not found...</div>
  }

  return (
    <div className='min-h-screen flex flex-col justify-center items-center gap-5 p-4 bg-gray-50'>
      <div className="flex justify-center items-center relative mb-4">
        <Image className="absolute p-5" src={assets.checkmark} alt='' width={80} height={80} />
        <div className="animate-spin rounded-full h-24 w-24 border-4 border-t-green-500 border-gray-200"></div>
      </div>
      <div className="text-center text-2xl font-semibold text-green-700 mb-4">Order Placed Successfully!</div>

      {/* Main Content Card */}
      <div className="bg-white shadow-lg rounded-lg p-6 w-full max-w-2xl">

        {/* Order Summary Table */}
        <h2 className="text-xl font-bold mb-3 text-orange-600 border-b pb-2">Order Summary</h2>
        <table className="w-full text-sm mb-4">
          ...
          <tbody>
            {/* Check array length */}
            {orderItems.length === 0 && (
              <tr><td colSpan={4} className="text-center py-4 text-gray-500">No products found in your order.</td></tr>
            )}
              ...
              return (
                <tr key={item.productId} className="border-b">
                  <td className="py-2 flex items-center gap-3">
                    <Image src={product.image[0]} alt={product.name} width={40} height={40} className="rounded w-10 h-10 object-cover border" />
                  ...
                  {/* Use item.quantity */}
                  <td className="py-2 text-center font-semibold">₹{product.offerPrice * item.quantity}</td>
                </tr>
              );
            })}
          </tbody>
        ...
        <div className="flex justify-end text-lg font-bold mb-6">
          Total: <span className="ml-2 text-orange-600">₹{orderItems.reduce((sum, item) => {
            const product = products.find(p => p._id === item.productId);
            // Use item.quantity
      ...
      <div className="text-gray-500 mt-4 text-sm">You will be redirected to My Orders in a few seconds...</div>
    </div>
  );
}
